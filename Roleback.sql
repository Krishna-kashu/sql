...
SELECT * FROM state_info_dup;
truncate table state_info_dup;

UPDATE state_info set state_id = 9999 where state_id = 2;
commit;
Rollback;
...
LIKE: %
% : can match any string of any length.

SELECT * FROM hospital_info where hospital_name 
like 'A%';
...
CREATE TABLE scam_info_dup as SELECT * FROM scam_info;
SELECT * FROM scam_info_dup;
DELETE FROM scam_info_dup WHERE scam_id=11;
DELETE FROM scam_info_dup WHERE state like 'M%';
Rollback;
